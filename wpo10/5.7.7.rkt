      ...
      (define (partition pivot i j)
        (let ((shifted-i (shift-to-right i pivot))
          ...
          (cond ((< shifted-i shifted-j)
                 ...
                 (partition pivot (+ shifted-i 1) (- shifted-j 1)))  ; Fixed: proper i,j progression
                (else shifted-j))))
      
      (define (m3-partition l r)
        (let ((middle (quotient (+ l r) 2)))
          (swap middle (+ l 1))
          (if (<<? (vector-ref vector l) (vector-ref vector (+ l 1)))     ; Fixed: added space
              (swap l (+ l 1)))
          (if (<<? (vector-ref vector r) (vector-ref vector (+ l 1)))     ; Fixed: added space
              (swap r (+ l 1)))
          (if (<<? (vector-ref vector r) (vector-ref vector l))           ; Fixed: added space
              (swap l r))
